...
import com.android.autelsdk.databinding.MissionCopyBinding
...
class MissionActivityCopy : AppCompatActivity(), View.OnClickListener {
    val viewModel: DFViewModel by viewModels()
    lateinit var binding: MissionCopyBinding

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = MissionCopyBinding.inflate(layoutInflater)
        ...
        when (id) {
            ...
            R.id.prepare -> {
                if (!viewModel.isDroneOk) {
                    Toast.makeText(this@MissionActivityCopy, "飞行器故障，不能执行", Toast.LENGTH_LONG).show()
                    return
                }
                viewModel.doPrepare()
            }
            ...
            R.id.start -> {
                if (viewModel.flyState != Prepare) {
                    Toast.makeText(this@MissionActivityCopy, "当前状态，不能执行", Toast.LENGTH_LONG).show()
                    return
                }
